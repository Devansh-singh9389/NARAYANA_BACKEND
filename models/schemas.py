...
class StyleConfig(BaseModel):
    ...
    flux_style_description: str = Field(...,
                                        description="A natural language sentence describing the art style, e.g., 'The image is illustrated in a highly detailed, 2D comic book style with sharp inking, vibrant flat colors, and deep dramatic shadows.'")
...
class CostumeOverride(BaseModel):
    ...
    body_override: Optional[str] = Field(default=None,
                                         description="Optional replacement for base_body_tags (e.g. for time-skips like adulthood)")
...
class SceneExtraction(BaseModel):
    ...
    previous_scene: Optional[int] = Field(default=None,
                                          description="ID of the previous scene if continuity is required")

    # UPDATED: Fused location and time into the single environment tag
    environment: str = Field(...,
                             description="Fused Danbooru tags for location, time, and lighting (e.g., 'city park, outdoors, morning, bright sunlight, autumn')")
    ...
    costume_overrides: list[CostumeOverride] = Field(default_factory=list,
                                                     description="Temporary outfit changes for this scene")
    visual: str = Field(...,
                        description="A highly detailed, natural language paragraph describing the exact visual composition. MUST include spatial relationships (e.g., 'In the foreground...', 'Behind the desk...'), exact lighting, and specific character actions. Write this as if describing a movie still to a blind person.")
